...
// Cue Blinker and Play Blinker
  ...
  WiresGroup
  {
    ...
    WiresGroup {
      ...
      // Wire { enabled: !module.shift; from: "%surface%.play"; to: ButtonScriptAdapter { color: Color.Green; brightness: deckPlayingProp.value } }
      Wire { from: "%surface%.play"; to: "VinylBreakTap" }
  ...
  Wire { from: "PlayBlinkerStopped.trigger"; to: ExpressionAdapter { type: ExpressionAdapter.Boolean; expression: module.active && !module.shift && !deckPlayingProp.value && (!deckCueingProp.value || (deckCueingProp.value && !deckRunningProp.value)) && (!tempoBendSteplessProp.value) && (!vinylBreakProp.value) && (deckIsLoaded.value || deckTypeProp.value == DeckType.Remix) } }
  ...
  Wire { from: "PlayBlinkerCueing.trigger"; to: ExpressionAdapter { type: ExpressionAdapter.Boolean; expression: module.active && !module.shift && ((!deckPlayingProp.value && deckCueingProp.value && deckRunningProp.value) || (tempoBendDiscreteProp.value) || (tempoBendSteplessProp.value) || (vinylBreakProp.value) || (vinylBreakLength_countdown.running)) && (deckIsLoaded.value || deckTypeProp.value == DeckType.Remix) } }
